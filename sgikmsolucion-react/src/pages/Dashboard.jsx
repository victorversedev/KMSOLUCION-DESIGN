...
import { FaCalendarDay, FaHandHoldingUsd, FaMoneyBillWave, FaExclamationTriangle } from 'react-icons/fa';
...
const fetchDashboardData = async () => {
  ...
  const [resFacturas, resCxc, resIngresos] = await Promise.all([
  ...
  const processDate = (dateStr) => {
    if (!dateStr) return { mes: 'S/N', quincena: 'S/N', semana: 'S/N', timestamp: 0 };
    const cleanStr = dateStr.split("T")[0];
    const [year, month, day] = cleanStr.split('-').map(Number);
    const d = new Date(year, month - 1, day);
    return {
      mes: `${mesesArr[d.getMonth()]} ${d.getFullYear()}`,
      quincena: `${day <= 15 ? '1Q' : '2Q'} ${mesesArr[month - 1]}`,
      semana: `Sem ${Math.ceil(day / 7)} ${mesesArr[month - 1]}`,
      timestamp: d.getTime()
    };
  };
  ...
  return {
    kpis: { 
      facturado: facturas.filter(i => esDeEsteMes(i.billing_date))
        .reduce((acc, item) => acc + parseFloat(item.amount || 0), 0),
      cobrado: ingresos.filter(i => esDeEstaSemana(i.payment_date || i.billing_date))
        .reduce((acc, item) => acc + parseFloat(item.amount || 0), 0),
      porCobrar: cxc.filter(i => esDeEsteMes(i.due_date))
        .reduce((acc, item) => acc + parseFloat(item.amount || 0), 0),
      carteraVencida: cxc.filter(i => esDeEsteMes(i.due_date) && String(i.status).toLowerCase().includes('vencid'))
        .reduce((acc, item) => acc + parseFloat(item.amount || 0), 0)
    },
    facturasRaw: facturas.map(item => ({ cliente: item.client_name?.toUpperCase() || 'S/N', categoria: item.category || 'General', monto: parseFloat(item.amount), ...processDate(item.billing_date) })),
    cobrarRaw: cxc.map(item => ({ cliente: item.client_name?.toUpperCase() || 'S/N', categoria: item.category || 'General', monto: parseFloat(item.amount), ...processDate(item.due_date) })),
    ingresosRaw: ingresos.map(item => ({ cliente: item.client_name?.toUpperCase() || 'S/N', categoria: item.category || 'General', monto: parseFloat(item.amount), ...processDate(item.payment_date || item.billing_date) }))
  };
};
...
const ZohoGraphic = ({ dataset, tipoGrafica, mostrarTendencia, nombreValor, colorPrincipal = ZOHO_BLUE }) => {
  ...
  useEffect(() => { 
    setOffset(Math.max(0, dataset.length - MAX_ITEMS)); 
  }, [dataset]);
  
  const fullDataList = useMemo(() => {
    return mostrarTendencia ? calcularLineaTendenciaMejorada(dataset, 'total') : dataset;
  }, [dataset, mostrarTendencia]);
  ...
  const renderContent = () => {
    const commonXAxis = <XAxis dataKey="ejeX" tick={{ fill: ZOHO_GREY, fontSize: 10 }} axisLine={{ stroke: '#ccc' }} tickLine={false} interval={0} angle={-45} textAnchor="end" height={60} />;
    const commonYAxis = <YAxis tickFormatter={(val) => `$${val.toLocaleString(undefined, { minimumFractionDigits: 2 })}`} tick={{ fill: ZOHO_GREY, fontSize: 11 }} axisLine={false} tickLine={false} width={90} />;
    const currencyFormatter = (value) => [`$${Number(value).toLocaleString(undefined, {minimumFractionDigits: 2})}`, nombreValor];
    ...
    switch (tipoGrafica) {
      case 'line': return (
          <LineChart data={visibleData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            ...
            <Line connectNulls type="monotone" dataKey="total" stroke={colorPrincipal} strokeWidth={3} dot={{ stroke: colorPrincipal, strokeWidth: 2, r: 4, fill: '#fff' }} name={nombreValor} />
            {mostrarTendencia && <Line connectNulls type="linear" dataKey="tendenciaRojaValue" stroke={ZOHO_RED} strokeWidth={2} dot={false} name="Tendencia Lineal" />}
          </LineChart>
        );
      case 'bar': return (
          <BarChart data={visibleData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            ...
            {mostrarTendencia && <Line connectNulls type="linear" dataKey="tendenciaRojaValue" stroke={ZOHO_RED} strokeWidth={2} dot={false} name="Tendencia" />}
          </BarChart>
        );
      case 'area': return (
          <AreaChart data={visibleData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            ...
            <RechartsTooltip formatter={currencyFormatter} />
            <Legend wrapperStyle={{ paddingTop: '20px' }} />
            <Area connectNulls type="monotone" dataKey="total" stroke={colorPrincipal} strokeWidth={2} fill={colorPrincipal} fillOpacity={0.15} name={nombreValor} />
            {mostrarTendencia && <Line connectNulls type="linear" dataKey="tendenciaRojaValue" stroke={ZOHO_RED} strokeWidth={2} dot={false} name="Tendencia" />}
          </AreaChart>
        );
      case 'pie': return (
          <PieChart>
            <Pie data={visibleData} cx="50%" cy="50%" innerRadius={70} outerRadius={110} paddingAngle={2} dataKey="total" nameKey="ejeX" label={({name, percent}) => `${name} (${(percent * 100).toFixed(2)}%)`}>
              {visibleData.map((_, index) => <Cell key={index} fill={PIE_COLORS[index % PIE_COLORS.length]} />)}
            </Pie>
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', display: 'flex', alignItems: 'center' }}>
      {fullDataList.length > MAX_ITEMS && <div onClick={() => canPrev && setOffset(offset - 1)} style={{ ...styles.arrowBtn, left: -10, opacity: canPrev ? 1 : 0.2, pointerEvents: canPrev ? 'auto' : 'none' }}><FiChevronLeft size={25} /></div>}
      ...
      {fullDataList.length > MAX_ITEMS && <div onClick={() => canNext && setOffset(offset + 1)} style={{ ...styles.arrowBtn, right: -10, opacity: canNext ? 1 : 0.2, pointerEvents: canNext ? 'auto' : 'none' }}><FiChevronRight size={25} /></div>}
    </div>
  );
};
...
export default function DashboardKMS() {
  ...
  const procesarGrafica = (rawKey, panelKey) => {
    ...
    return Object.values(groupedMap).sort((a, b) => a.sortKey - b.sortKey);
  };
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <div style={styles.cardHeaderControls}>
          ...
          <div style={styles.controlsRow}>
            ...
            <select style={styles.selectSmall} value={filtros[panel].agrupar} onChange={(e) => handleFiltroChange(panel, 'agrupar', e.target.value)}>
              <option value="mes">Mes</option>
              <option value="quincena">15D</option>
              <option value="semana">Sem</option>
            </select>
            <select style={styles.selectSmall} value={filtros[panel].tipo} onChange={(e) => handleFiltroChange(panel, 'tipo', e.target.value)}>
        ...
        <div style={{ flex: 1, minHeight: '300px' }}>
          <ZohoGraphic 
            ...
            mostrarTendencia={['line', 'bar', 'area'].includes(filtros[panel].tipo)} 
  ...
  if (isLoading) return <div className="layout-container"><SideMenu /><div className="main-content"><TopBar /><div style={styles.center}><h3>Cargando métricas...</h3></div></div></div>;
  if (error) return <div className="layout-container"><SideMenu /><div className="main-content"><TopBar /><div style={styles.center}><h3>Error de conexión.</h3></div></div></div>;
      ...
      <div className="main-content">
        ...
        <main className="content-area" style={{ backgroundColor: '#f4f5f7', padding: '24px' }}>
          <div style={styles.header}>
            ...
            <p style={{ color: '#6b7280', margin: '5px 0 0 0' }}>Panel Administrativo</p>
          </div>
          <div style={styles.kpiGrid}>
            <KpiCard title="Por Facturar en el mes" value={`$${data.kpis.facturado.toLocaleString(undefined, {minimumFractionDigits: 2})}`} icon={<FaCalendarDay />} color="#6366f1" />
            <KpiCard title="Ingreso Semanal" value={`$${data.kpis.cobrado.toLocaleString(undefined, {minimumFractionDigits: 2})}`} icon={<FaHandHoldingUsd />} color={ZOHO_GREEN} />
            <KpiCard title="CxC" value={`$${data.kpis.porCobrar.toLocaleString(undefined, {minimumFractionDigits: 2})}`} icon={<FaMoneyBillWave />} color="#f59e0b" />
            <KpiCard title="Cartera Vencida" value={`$${data.kpis.carteraVencida.toLocaleString(undefined, {minimumFractionDigits: 2})}`} icon={<FaExclamationTriangle />} color={ZOHO_RED} />
          </div>
...
const styles = {
  ...
  card: { background: 'white', padding: '24px', borderRadius: '8px', border: '1px solid #e5e7eb', boxShadow: '0 1px 2px rgba(0,0,0,0.05)', cursor: 'default' },
  ...
  select: { maxWidth: '140px', padding: '6px 10px', borderRadius: '6px', border: '1px solid #d1d5db', fontSize: '0.85rem' },
  selectSmall: { maxWidth: '80px', padding: '6px 10px', borderRadius: '6px', border: '1px solid #d1d5db', fontSize: '0.85rem' },
